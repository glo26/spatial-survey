...
% NeurIPS-style formatting (matching "Attention Is All You Need")
\usepackage[utf8]{inputenc}
% \usepackage[T1]{fontenc}  % Removed for compatibility
% Using default Computer Modern fonts for compatibility
...
% Title formatting
\title{\Large \bf Agentic AI for Spatial Intelligence: A Comprehensive Survey}

% Author formatting (matching Attention Is All You Need style)
\author{
Gloria Felicia\thanks{gloria.felicia@atlaspro.ai} \quad
Handi Putra\thanks{handi.putra@atlaspro.ai} \quad
Ayaan Gazali\thanks{ayaan.gazali@atlaspro.ai} \\
Eliel Lobo\thanks{eliel.lobo@atlaspro.ai} \quad
Esteban Rojas\thanks{esteban.rojas@atlaspro.ai} \\[1em]
AtlasPro AI
}
...
\textbf{Vision-Language Navigation (VLN).} Agents that follow natural language instructions to navigate real-world environments \citep{anderson2018vln, chen2019touchdown, hong2020vlnbert, zhu2019vision}.

\textbf{Embodied Question Answering (EQA).} Agents that must explore an environment to find the answer to a question \citep{das2018eqa}.

\textbf{Robotic Manipulation.} Agents that can manipulate objects to achieve goals, often involving complex spatial reasoning and planning, as seen in the SayCan system \citep{ahn2022saycan} and VIMA \citep{lin2022vima}.
...
GNNs are well-suited for modeling spatial relationships. Spatio-Temporal GNNs have been successfully applied to urban computing tasks like traffic forecasting \citep{li2018dcrnn, yu2018stgcn, wu2019graphwavenet, jiang2022gnn}. Graph Transformers \citep{shehzad2024graphtransformers} offer a scalable approach to capturing long-range spatial dependencies, making them suitable for large-scale spatial graphs like road networks.
...
\textbf{Navigation Benchmarks.} Datasets like R2R \citep{anderson2018vln} and Habitat \citep{savva2019habitat} evaluate navigation capabilities.

\textbf{Manipulation Benchmarks.} Environments like ALFWorld \citep{yao2021alfworld} and BEHAVIOR \citep{srivastava2021behavior} test object manipulation and task completion.

\textbf{Spatial Reasoning Benchmarks.} Datasets like CLEVR \citep{johnson2017clevr} and GQA \citep{hudson2019gqa} assess compositional spatial reasoning.

\textbf{Integrated Agent Benchmarks.} Recent benchmarks like AgentBench \citep{liu2023agentbench}, EmbodiedBench \citep{yang2025embodiedbench}, and REM \citep{thompson2025rem} evaluate multiple agent capabilities in complex environments.
