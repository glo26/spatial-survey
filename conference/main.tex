...
We adopt the established design pattern formalism from software architecture \citep{gamma1994design}: each pattern specifies a \textit{problem} (recurring challenge), \textit{forces} (constraints that shape solutions), and \textit{solution} (reusable architectural template).

\subsection{Design Pattern 1: Human-in-the-Loop Spatial Reasoning}

\textit{Problem:} High-stakes spatial decisions require accuracy beyond current AI capabilities. \textit{Forces:} Accountability requirements, liability concerns, domain expertise scarcity. \textit{Solution:} AI proposes spatial analyses; humans validate and correct; corrections update system memory or policy.
...
\textbf{Geospatial Intelligence.} Defense-grade geospatial fusion systems \citep{palantir2023, palantir2024aip} integrate multi-source spatial data (satellite imagery, signals intelligence, terrain models) with human analyst workflows. These systems enable intelligence analysis with spatial reasoning while maintaining human oversight for critical decisions---a requirement driven by accountability and legal constraints in defense contexts.

\textbf{GIS Workflows.} ESRI \citep{esri2023, esri2024geoai, esri2024arcgis} provides ArcGIS with integrated GeoAI capabilities where AI assists human planners in urban planning, environmental monitoring, and disaster response. The workflow follows a reusable 3-step schematic: (1) \textit{Agent proposal}---AI generates spatial analysis using memory-augmented reasoning; (2) \textit{Human validation/correction}---domain expert reviews, accepts, or modifies outputs; (3) \textit{Memory update}---corrections are stored for retrieval-augmented future queries, enabling continuous improvement without full model retraining.

\subsection{Design Pattern 2: Weakly Supervised Planetary-Scale Learning}

\textit{Problem:} Macro-scale spatial reasoning requires global coverage, but dense labeling is infeasible at planetary scale. \textit{Forces:} Label scarcity (only 0.01\% of Earth's surface has dense annotations), distribution shift across geographies, and computational constraints. \textit{Solution:} Self-supervised pretraining on petabyte-scale imagery, followed by task-specific fine-tuning with minimal labels.
...
\textbf{Satellite Foundation Models.} The common pipeline abstracts as: \textit{Data} (multi-spectral satellite imagery from Landsat, Sentinel, commercial providers) $\rightarrow$ \textit{Representation} (self-supervised pretraining via masked autoencoding or contrastive learning) $\rightarrow$ \textit{Downstream reasoning} (fine-tuning for land cover classification, change detection, or crop yield prediction). NASA-IBM Prithvi \citep{jakubik2024prithvi} exemplifies this pipeline with Harmonized Landsat Sentinel-2 data. Planet Labs \citep{planet_labs_2023, planet2024daily} and Maxar provide the data infrastructure enabling daily global monitoring.
...
\textit{Contrast with HITL:} In Pattern 1 (HITL), the human is the \textit{gatekeeper}---every AI output requires validation. In Pattern 3, the agent is the \textit{primary operator}---humans supervise and intervene only when confidence is low or stakes are high.
...
\textit{Problem:} Deploying spatial AI in safety-critical physical systems. \textit{Forces:} Regulatory requirements (NHTSA, EU AI Act), liability frameworks, long-tail safety scenarios, and public trust. \textit{Solution:} Massive simulation for policy learning, conservative real-world deployment with human oversight, continuous learning from fleet data with safety constraints.
...
\textbf{Autonomous Vehicles.} Two contrasting sensing philosophies have emerged: \textit{Lidar-centric stacks} (Waymo \citep{waymo2023, waymo_emma_2024, waymo2024driver}, Cruise) fuse lidar point clouds with camera imagery for redundant depth estimation, achieving higher safety margins but at greater sensor cost. \textit{Vision-only stacks} (Tesla \citep{tesla2023fsd, tesla2024autopilot}) rely on camera arrays with learned depth estimation, enabling lower hardware cost but requiring more robust perception models. EMMA \citep{waymo_emma_2024} represents a hybrid approach with end-to-end multimodal models. The common pattern: massive simulation, conservative real-world deployment, continuous learning from fleet data under regulatory oversight.
...
\begin{itemize}[leftmargin=*,nosep]
    \item \textbf{Design for human oversight:} Safety-critical spatial systems require HITL patterns; plan for human validation interfaces from the start
    \item \textbf{Embrace weak supervision:} Planetary-scale spatial reasoning is only feasible with self-supervised pretraining; dense labeling does not scale
    \item \textbf{Calibrate autonomy to stakes:} Use agent-assisted (high autonomy) for routine tasks, HITL (low autonomy) for high-stakes decisions
    \item \textbf{Simulate before deploying:} Embodied spatial AI requires orders of magnitude more simulation than real-world data; budget accordingly
\end{itemize}
\end{tcolorbox}
